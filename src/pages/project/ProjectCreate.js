    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:8000/api/task/', {
                ...
                body: array, // Ensure the data is properly stringified
            });
            ...
            if (response.ok) {
                ...
                const array = JSON.stringify({
                    start_date: formattedDate, // Pass the formatted date
                    start_time: formattedTime, // Pass the formatted time
                });

                const points = await fetch('http://127.0.0.1:8000/api/task/' + result.id + '/task-time-info/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    ...
                    <section className="row padding-3em bg-viwe ">
                        <div className='flex justify-content-between margin-bottom-2em'>
                            <h3 className="title">
                                ...
                                <span className="margin-left">업무 생성</span>
                            </h3>
                            <div className='flex'>
