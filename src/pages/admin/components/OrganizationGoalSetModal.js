...
const OrganizationGoalSetModal = ({ onClose, data }) => {
    ...
    const initialState = {
        ...
        goal_type: data?.goal_type || 'long_term',
        title: data?.title || 'Default title',  // Example of providing a default title
        content: data?.content || 'No content provided.',
        result: data?.result || 'Pending',
        organization: data
    };

    const { formState, mainUrl, handleChange, handleSubmit } = useForm(initialState, async (json) => {
        try {
            const response = await fetch(`${mainUrl}/api/accounts/organization-goal/`, {
                method: "POST",
                headers: {
    ...
    const list = useCallback(async () => {
        const response = await fetch(`${mainUrl}/api/accounts/organization-goal/?organization=1`, {
            method: "GET",
            headers: {
                "Authorization": `Token ${localStorage.getItem('token')}`,
                "Content-Type": "application/json"
    ...
    const deleteOn = async (id) => {
        try {
            const response = await fetch(`${mainUrl}/api/accounts/organization-goal/${id}/`, {
                method: "Delete",
                headers: {
                    "Authorization": `Token ${localStorage.getItem('token')}`,
                    "Content-Type": "application/json"
                ...
                <div className="p-4 md:p-5 grid grid-cols-1 lg:grid-cols-5 gap-4">
                    <form className="space-y-4 2xl:col-span-2" onSubmit={handleSubmit}>
                        <div className="col-span-2">
                            <label htmlFor="organization-goal-start_date" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">기간</label>
                            <div className="flex items-center">
                                <input
                                    type="date"
                                    id={`organization-goal-start_date`}
                                    name={`start_date`}
                                    value={formState.start_date}
                                    onChange={handleChange}
                                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-1 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" />
                                <span className="px-2">~</span>
                                <input
                                    type="date"
                                    id={`organization-goal-end_date`}
                                    name={`end_date`}
                        ...
                        <div className="col-span-2">
                            <label htmlFor="organization-goal-title" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">기간별 목표</label>
                            <input
                                type="text"
                                id={`organization-goal-title`}
                                name={`title`}
                                value={formState.title}
                                onChange={handleChange}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" />
                            {/* <select
                                id="organization-goal-goal_type"
                                name="goal_type"
                                value={formState.goal_type}
                                onChange={handleChange}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                                <option value="long_term">장기 목표</option>
                                <option value="mid_term">중기 목표</option>
                                <option value="monthly">월별-실행목표</option>
                                <option value="weekly">주별-실행목표</option>
                            </select> */}
                        </div>
                        <div className="col-span-2">
                            <label htmlFor="organization-goal-content" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">내용</label>
                            <textarea
                                type="text"
                                rows={5}
                                id={`organization-goal-content`}
                                name={`content`}
                        ...
                        <div className="col-span-2">
                            <label htmlFor="organization-goal-result" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">목표 결과</label>
                            <input
                                type="text"
                                name="result"
                                id="organization-goal-result"
                    ...
                    <div className="2xl:col-span-3">
                        <h3 className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">목표 목록</h3>
...
export default OrganizationGoalSetModal;