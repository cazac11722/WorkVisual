...
const ReasonSetModal = ({ onClose, data, setData }) => {
    ...
    const { formState, mainUrl, handleChange, handleSubmit } = useForm(
        { name: "", content: " ", organization: data.id },
        async (json) => {
            try {
                const response = await fetch(`${mainUrl}/api/accounts/organization-reasons/`, {
                    method: "POST",
                    headers: {
    ...
    const deleteOn = async (id) => {
        try {
            const response = await fetch(`${mainUrl}/api/accounts/organization-reasons/${id}/`, {
                method: "Delete",
                headers: {
                    "Authorization": `Token ${localStorage.getItem('token')}`,
                    "Content-Type": "application/json"
                ...
                <div className="p-4 md:p-5 grid grid-cols-1 lg:grid-cols-5 gap-4">
                    <form className="space-y-4 2xl:col-span-2" onSubmit={handleSubmit}>
                        <div className="col-span-2">
                            <label htmlFor="reason_name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">제목</label>
                            <input type="text" name="name" id="reason_name" value={formState.name} onChange={handleChange} required={true} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" />
                        </div>
                        <div className="col-span-2">
                            <label htmlFor="reason_content" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">내용</label>
                            <textarea
                                type="text"
                                rows={5}
                                id={`reason_content`}
