...
const RankSetModal = ({ onClose, data, setData }) => {
    ...
    const { formState, mainUrl, handleChange, handleSubmit } = useForm(
        ...
        async (json) => {
            try {
                const response = await fetch(`${mainUrl}/api/accounts/organization-ranks/`, {
                    method: "POST",
                    headers: {
                ...
                if (response.ok) {
                    ...
                    setChData((prev) => ({
                        ...prev,
                        rank_list: [...(prev.rank_list || []), result]
                    }));
                    setData((prev) => ({
                        ...prev,
                        rank_list: [...(prev.rank_list || []), result]
    ...
    const deleteOn = async (id) => {
        try {
            const response = await fetch(`${mainUrl}/api/accounts/organization-ranks/${id}/`, {
                method: "Delete",
                headers: {
                    "Authorization": `Token ${localStorage.getItem('token')}`,
                    "Content-Type": "application/json"
            ...
            if (response.ok) {
                ...
                setChData((prev) => ({
                    ...prev,
                    rank_list: prev.rank_list.filter(dept => dept.id !== id)
                }));
                setData((prev) => ({
                    ...prev,
                    rank_list: prev.rank_list.filter(dept => dept.id !== id)
    ...
    // ✅ 페이지네이션 계산
    const totalPages = Math.ceil((chData.rank_list?.length || 0) / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const filteredDepartments = chData.rank_list?.slice(startIndex, endIndex) || [];
            ...
            <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div className="p-4 md:p-5 grid grid-cols-1 lg:grid-cols-5 gap-4">
                    ...
                    <div className="2xl:col-span-3">
                        <h3 className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">직급/등급 목록</h3>
