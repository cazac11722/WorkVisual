...
const OrganizationProjectTypeSetModal = ({ onClose, data }) => {
    ...
    const { formState, mainUrl, handleChange, handleSubmit } = useForm(
        ...
        async (json) => {
            try {
                const response = await fetch(`${mainUrl}/api/accounts/organization-project-type/`, {
                    method: "POST",
                    headers: {
    ...
    const list = useCallback(async () => {
        const response = await fetch(`${mainUrl}/api/accounts/organization-project-type/?organization=1`, {
            method: "GET",
            headers: {
                "Authorization": `Token ${localStorage.getItem('token')}`,
                "Content-Type": "application/json"
    ...
    const deleteOn = async (id) => {
        try {
            const response = await fetch(`${mainUrl}/api/accounts/organization-project-type/${id}/`, {
                method: "Delete",
                headers: {
                    "Authorization": `Token ${localStorage.getItem('token')}`,
                    "Content-Type": "application/json"
            ...
            <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div className="p-4 md:p-5 grid grid-cols-1 lg:grid-cols-5 gap-4">
                    ...
                    <div className="2xl:col-span-3">
                        <h3 className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">프로젝트 유형 목록</h3>
...
export default OrganizationProjectTypeSetModal;