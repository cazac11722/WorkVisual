...
const SidebarItemSub = ({ href, icon, name, count, textShow }) => {
    ...
    return (
        <div className="">
            <button type="button" onClick={toggleOpen} className="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-700">
                ...
                <IconWidget icon={'ArrowDown'} className={`fill-black transition ${textShow ? 'hidden' : ''} ${ isOpen ? 'rotate-180' : ''}`} />
            </button>
            <div className={`overflow-hidden ${ isOpen ? 'h-auto' : 'h-0'}`}>
                <Link to={href} className="flex items-center p-2 pl-8 text-base text-gray-900 rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700">
                    <span className={`ml-3 ${textShow ? 'hidden' : ''}`}  >직원 정보 관리</span>
                ...
                <Link to={href} className="flex items-center p-2 pl-8 text-base text-gray-900 rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700">
                    <span className={`ml-3 ${textShow ? 'hidden' : ''}`}  >직원 프로젝트 관리</span>
                    {count > 0 ? <span className="flex items-center justify-center ml-auto w-7 h-7 text-xs text-white bg-red-500 rounded-full ">{count}</span> : ''}
                </Link>
                <Link to={href} className="flex items-center p-2 pl-8 text-base text-gray-900 rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700">
                    <span className={`ml-3 ${textShow ? 'hidden' : ''}`}  >직원 통계 및 분석</span>
                    {count > 0 ? <span className="flex items-center justify-center ml-auto w-7 h-7 text-xs text-white bg-red-500 rounded-full ">{count}</span> : ''}
                </Link>
                <Link to={href} className="flex items-center p-2 pl-8 text-base text-gray-900 rounded-lg hover:bg-gray-100 group dark:text-gray-200 dark:hover:bg-gray-700">
                    <span className={`ml-3 ${textShow ? 'hidden' : ''}`}  >설정</span>
                    {count > 0 ? <span className="flex items-center justify-center ml-auto w-7 h-7 text-xs text-white bg-red-500 rounded-full ">{count}</span> : ''}
                </Link>
            </div>
        </div>
